...
const VideoPlayerControls: React.FC<VideoPlayerControlsProps> = ({
  ...
  <div className='group relative flex justify-center items-center sm:opacity-0 group-hover:opacity-100 transition-all duration-200'>
    <svg width={size} height={size} style={{transform: "rotate(-90deg)"}}>
      ...
      <circle cx={center} cy={center} r={radius} fill='transparent' stroke='#6c6c6c' strokeWidth={width} strokeDasharray={dashArray} strokeDashoffset={dashOffset} />
    </svg>
    <div className='absolute'>
      <button onClick={onPlayPause} className='group cursor-pointer flex justify-center items-center'>
        <div className='transition-colors duration-200 sm:hover:fill-black fill-black/50 ease-in-out'>
