...
const articlesPath = path.join(process.cwd(), "misc_pages");
...
export async function getAllPages() {
  const articles = fs.readdirSync(path.join(process.cwd(), "/misc_pages"));

  return articles.reduce((allArticles, articleSlug) => {
    // get parsed data from mdx files in the "articles" dir
    const source = fs.readFileSync(
      path.join(process.cwd(), "/misc_pages", articleSlug),
      "utf-8"
    );
